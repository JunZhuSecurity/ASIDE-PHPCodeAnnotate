...
public static void writeConciseWarnedCodeIntoFile(HashSet<ConciseWarnedCode> set){
		
		String fileName = Plugin.STATE_LOCATION + "/" + Plugin.CONCISE_WARNED_CODE_FILE;
		
		System.out.println("size of markers =========== " + Plugin.allMarkerRecords.size());
		Iterator<ConciseWarnedCode> iter = set.iterator();
		ConciseWarnedCode tmp = null;
		String fileDir = null;
		int lineNum = 0;
		...
		while(iter.hasNext()){
			...
			tmp = iter.next();
			fileDir = tmp.getFileDir();
			lineNum = tmp.getLineNum();
			nodeName = tmp.getNodeName();
			//store the Marker information in the persistent file
			String str = nodeName + seperator + lineNum + seperator + fileDir + "\n";
			strBuf.append(str);
		//	System.out.println("fileDir=" + fileDir + ", nodeStart=" + nodeStart + ",nodeLength=" + nodeLength + ", markerType=" + markerType + ",isAnnotated=" + isAnnotated);
		}
